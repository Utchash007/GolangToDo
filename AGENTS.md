...
internal/
  ...
  middleware/
    ...
    recovery.go                  # panic recovery → slog.Error with stack + request_id → 500
  task/
    model.go                     # Task struct, Priority enum (Value/Scan/JSON), DTOs, ValidationError, FieldError, ErrorResponse
    ...
    handler.go                   # Gin handlers; bindingErrors translates validator errors; handleError maps to 400/404/500
migrations/
...
- Unified `ErrorResponse{code, errors:[]FieldError}` shape for all errors — `FieldError.Field` is omitempty (absent for non-field errors)
- `bindingErrors(err)` translates Gin/validator errors into `[]FieldError` via `errors.As` (not direct type assertion)
- Panic recovery middleware registered first — logs stack + request_id via slog, returns 500
